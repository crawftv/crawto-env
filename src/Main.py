...
class RequirementsInWriter:
    ...
    def check_for_in(self) -> bool:
        if not os.path.exists("requirements.in"):
            response = input("Cannot find requirements.in file.\nDo you want to create one in this directory? y/n")
            if response.lower() =="y":
    ...
    def write(self,packages):
        ...
        with click.open_file(self.dst_file,"w",atomic=True, lazy=True) as requirements_in:
...
def install(packages):
    ...
    os.system("pip-compile requirements.in --output-file=requirements.txt; pip install -r requirements.txt")
    return
